...
public class WordCountProcessorApplicationTests {
	...
	public static final String INPUT_TOPIC = DemoKafkaStreamApplication.WordCountProcessorApplication.INPUT_TOPIC;
	public static final String OUTPUT_TOPIC = DemoKafkaStreamApplication.WordCountProcessorApplication.OUTPUT_TOPIC;

	final Serde<String> stringSerde = Serdes.String();
	final JsonSerde<DemoKafkaStreamApplication.WordCount> countSerde = new JsonSerde<>(DemoKafkaStreamApplication.WordCount.class);
	...
	public void setup() {
		...
		DemoKafkaStreamApplication.WordCountProcessorApplication app = new DemoKafkaStreamApplication.WordCountProcessorApplication();
		final Function<KStream<Bytes, String>, KStream<Bytes, DemoKafkaStreamApplication.WordCount>> process = app.process();

		final KStream<Bytes, DemoKafkaStreamApplication.WordCount> output = process.apply(input);
	 ...
	 */
	private ProducerRecord<Bytes, DemoKafkaStreamApplication.WordCount> readOutput() {
	...
	private Map<String, Long> getOutputList() {
		...
		ProducerRecord<Bytes, DemoKafkaStreamApplication.WordCount> outputRow;
	...
	public void testOneWord() {
		...
		final ProducerRecord<Bytes, DemoKafkaStreamApplication.WordCount> output = readOutput();
		assertThat(output).isNotNull();
		assertThat(output.value()).isEqualToComparingFieldByField(new DemoKafkaStreamApplication.
				WordCount("hello", 1L, new Date(0), new Date(DemoKafkaStreamApplication.WordCountProcessorApplication.WINDOW_SIZE_MS)));
